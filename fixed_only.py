...
class PostAnalysis:
    def __init__(self, res_filepath, cutoff, ans_filepath=None, output_dir='rag/analysis_results'):
    ...
    def extract_intermediate_queries(self):
        ...
        for _,res in res_df.iterrows():
            ...
            # Parse the output to extract the intermediate queries here
            matches = re.findall(search_pattern, output, flags=re.DOTALL)

            extracted_data[qid] = {
                "query": query,
                "inter_queries": matches[:self.cutoff],
...
Answer:"""
    ...
    def generating_answers(self):
        logger.info(f"Starting analysis with cutoff: {self.cutoff}")
    ...
    def save_results(self, _ret="post_analysis", _k=None, _task='nq_test', _model='generator'):
        ...
        json_filename = f"{self.output_dir}/analysis_results_cutoff_{self.cutoff}_{timestamp}.json"
        ...
        if self.ans_data is not None:
            ...
            csv_filename = f"{self.output_dir}/analysis_summary_cutoff_{self.cutoff}_{timestamp}.csv"
